...
**What it is:** Groups points by density. This finds arbitrarily shaped clusters without specifying K upfront.

**Pros:**

- ✅ No need to specify number of clusters
- ✅ Good at finding outliers
- ✅ Knitted fabric has 3D structure, where stitches are raised and gaps are recessed, creating a sharp light-to-dark transition rather than a smooth gradient. This means shadow tones in the gaps (e.g. dark purple between light purple stitches, see [soft-purple yarn](../../examples/yarn/soft-purple-yarn.jpg)) can form their own dense region in colour space, separated by a sparse transition zone. DBSCAN could detect these as a separate cluster, which would be useful for realistic garment recolouring by mapping garment shadows to yarn shadow tones rather than only the top-K dominant colours

**Cons:**

- ❌ Cannot control exact number of output colours ie. may return 2 or 20
- ❌ Struggles with varying density, ie. yarn images with large areas of one colour (e.g. 80% navy) and tiny specks of another (e.g. 20% gold flecks). DBSCAN finds the dense navy cluster easily but may label the sparse gold pixels as noise. Lowering `eps`/`min_samples` to catch the gold risks splitting navy into multiple sub-clusters. No single parameter setting handles both dense and sparse regions in the same image.
...
- ✅ Produces a hierarchy. Thus, can cut at any level for different K
...
- ❌ K-means only computes distances between each pixel and K centroids, so O(n × K) per iteration
...
- ✅ More flexible to handle elliptical clusters
- ✅ Soft assignments (probability per cluster)

**Cons:**

- ❌ Slower than K-means (EM algorithm with covariance estimation)
- ❌ Extra flexibility not useful here, as we need a definite "what are the 5 dominant colours", not probability distributions
...
**Where this breaks down with a marled/twisted yarn example:**
...
2. **Fractional counting** - a 55/45 pixel contributes 0.55 to navy and 0.45 to cream. More accurate percentages in theory, but the recolouring pipeline maps discrete pixels to discrete colours. Therefore, it makes no sense to recolour a pixel 55% navy and 45% cream. You still have to pick one.
3. **Create a new cluster** - treat the ambiguous zone as its own colour. But then you extract 6 colours instead of 5, and the extra colour is a blend that does not actually exist in the yarn, which is an optical artifact of twisted plies.
...
- **2026-04-14:** ✅ n_clusters increased from 5 to 10 in web app to capture shadow tones for single-colour yarn input
