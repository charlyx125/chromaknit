...
| Data | Where | Why |
|---|---|---|
| Yarn palette (`yarns[]`) | localStorage | User-owned, durable across sessions |
| `activeYarnId` | not persisted | Working state; resets each session is fine |
| Garment image + rembg mask | server session (slice 1.B) | Expensive to recompute; ephemeral by design |
| Recolour result blobs | browser memory (slice 1.C) | Cheap to recompute via API |
...
The split: **persistence** for user-owned data; **caching** for expensive-to-recompute artifacts. Yarn palettes are persistence; everything else is caching.
