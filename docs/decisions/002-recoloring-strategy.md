...
| **Multi-colour source garments** | The HSV remap assumes one base colour plus brightness variation from texture and lighting. On striped or fair-isle source photos, contrasting colours are read as shadow and highlight bands and collapsed onto the yarn's brightness range | Output is internally consistent but the original colour structure is lost. A blue-and-cream striped source becomes a single recoloured surface with no stripe visible | Paint mode lets users segment colour regions by hand. Phase 3 Select mode targets automatic colour-aware segmentation (Lab-distance flood fill) |
