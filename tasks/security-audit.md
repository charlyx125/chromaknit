...
| §1 tracked `.env.development`/`.env.production` | medium | low | **resolved** | [chromaknit-frontend/.gitignore](chromaknit-frontend/.gitignore) no longer lists `.env.production`; both tracked public-value mode files are now consistent with the ignore rule. Adds `.env.*.local` to cover Vite's per-mode local-override pattern. Verified via `git check-ignore`: only `.env.local` and `.env.*.local` are ignored; `.env.development` and `.env.production` are not. See commit `resolve gitignore vs tracked .env files contradiction (SECURITY.md §1)`. |
...
| §5 no rate limit | high | medium | **resolved** | slowapi `Limiter` wired in [api/main.py](api/main.py); 60/minute per-IP cap on `/api/colors/extract`, `/api/garments/session`, `/api/garments/recolor`. Returns 429 with `Retry-After` once exceeded. Test `test_extract_colors_enforces_per_ip_rate_limit` sends 61 requests and asserts the 61st returns 429. See commit `add per-IP rate limit on POST endpoints (SECURITY.md §5)`. |
...
| §8 backend unpinned deps | medium-high | medium | **resolved (partial)** | `requirements.lock` generated by `pip-compile`; [Dockerfile](Dockerfile) and [.github/workflows/tests.yml](.github/workflows/tests.yml) now install from the lock. Version pins only, no hashes; the gap is documented in SECURITY.md §8 (dev Python 3.13 vs prod 3.11 wheel hashes do not match; revisit when stakes change). See commit `pin backend deps via requirements.lock (SECURITY.md §8)`. |
...
| §9 hardcoded Vercel preview origins | low | low | **resolved** | `VERCEL_ORIGIN_REGEX` added in [api/main.py](api/main.py); anchored on `charlyx125` so unrelated `chromaknit-*` projects under other Vercel accounts cannot match. Literal allowlist now covers only localhost, huggingface.co, and the Space's own origin. Tests `test_cors_accepts_known_origins` (7 shapes) and `test_cors_rejects_unknown_origins` (6 shapes including prefix-injection attempts). See commit `widen Vercel preview CORS via anchored regex (SECURITY.md §9)`. |
