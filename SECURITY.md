...
- **Backend (Python) must have a hashed lockfile.** Generate with `pip-compile --generate-hashes` (from `pip-tools`) or equivalent, commit `requirements.lock`, and install from it in both the Dockerfile and CI. Today `requirements.txt` uses `>=` floors with no upper bound and no lock; every Docker rebuild re-resolves and can pick up a malicious upstream release.
